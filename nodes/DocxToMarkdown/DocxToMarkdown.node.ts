...
export class DocxToMarkdown implements INodeType {
    ...
    properties: [
      {
        ...
        default: "data",
        description:
          "The name of the input binary property that contains the .docx file",
      },
      {
      ...
      },
      {
        ...
        default: false,
        displayOptions: { show: { outputMode: ["json"] } },
        description: "Also include HTML version in the JSON output",
      },
      {
        ...
        type: "boolean",
        default: true,
        description: "Try to preserve headings, lists, and tables",
      },
      {
  ...
  async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      const outputMode = this.getNodeParameter("outputMode", i) as
        | "json"
        | "binary";
      ...
      const preserveStructure = this.getNodeParameter(
        "preserveStructure",
        i
      ) as boolean;
      ...
      if (!looksDocx) {
        throw new Error(
          `Item ${i}: Expected a .docx file (mime="${mime}", name="${
            inputBinary.fileName ?? "unknown"
          }")`
        );
      }
      ...
      const mammothOptions = {
        ...
        convertImage: mammoth.images.imgElement(async (image) => {
          ...
          const dataUri = `data:${type};base64,${Buffer.from(buf).toString(
            "base64"
          )}`;
          return { src: dataUri };
        }),
